#!/usr/bin/env python3
"""
Gold Variations Renderer

Renders multiple gold material variations as sphere previews using Blender.
Run with: /path/to/blender --background --python render-gold-variations.py

Output: Creates 512x512 PNG images for each gold variation
"""
...
OUTPUT_DIR = Path(__file__).parent.parent / "public" / "assets" / "materials" / "metal" / "gold-variations"

# Gold variations to render
GOLD_VARIATIONS = [
    # Standard Golds
    {"id": "gold-mirror", "name": "Mirror Gold", "color": (1.0, 0.843, 0.0), "roughness": 0.0, "metalness": 1.0},
    {"id": "gold-polished", "name": "Polished Gold", "color": (1.0, 0.843, 0.0), "roughness": 0.1, "metalness": 1.0},
    {"id": "gold-satin", "name": "Satin Gold", "color": (1.0, 0.843, 0.0), "roughness": 0.3, "metalness": 1.0},
    {"id": "gold-brushed", "name": "Brushed Gold", "color": (1.0, 0.843, 0.0), "roughness": 0.45, "metalness": 1.0},
    {"id": "gold-matte", "name": "Matte Gold", "color": (1.0, 0.843, 0.0), "roughness": 0.6, "metalness": 1.0},

    # Gold Tints
    {"id": "gold-rose", "name": "Rose Gold", "color": (0.72, 0.43, 0.47), "roughness": 0.15, "metalness": 1.0},
    {"id": "gold-white", "name": "White Gold", "color": (0.96, 0.96, 0.96), "roughness": 0.1, "metalness": 1.0},
    {"id": "gold-champagne", "name": "Champagne Gold", "color": (0.97, 0.91, 0.81), "roughness": 0.15, "metalness": 1.0},
    {"id": "gold-rich", "name": "Rich Gold", "color": (1.0, 0.76, 0.15), "roughness": 0.15, "metalness": 1.0},
    {"id": "gold-pale", "name": "Pale Gold", "color": (0.9, 0.76, 0.53), "roughness": 0.15, "metalness": 1.0},

    # Aged & Special
    {"id": "gold-aged", "name": "Aged Gold", "color": (0.81, 0.71, 0.23), "roughness": 0.4, "metalness": 0.9},
    {"id": "gold-antique", "name": "Antique Gold", "color": (0.73, 0.55, 0.19), "roughness": 0.5, "metalness": 0.85},
    {"id": "gold-dark", "name": "Dark Gold", "color": (0.67, 0.42, 0.22), "roughness": 0.2, "metalness": 1.0},
    {"id": "gold-worn", "name": "Worn Gold", "color": (0.85, 0.65, 0.15), "roughness": 0.55, "metalness": 0.8},
    {"id": "gold-rough", "name": "Rough Cast Gold", "color": (1.0, 0.843, 0.0), "roughness": 0.8, "metalness": 1.0},
]
...
def setup_scene():
    ...
    # Try to load a studio HDRI from Blender's bundled assets
    hdri_path = None
...
def create_gold_material(variation):
    """Create a PBR gold material"""
...
def render_variation(sphere, variation, output_dir):
    """Render a single gold variation"""
    # Create and apply material
    mat = create_gold_material(variation)
...
def main():
    ...
    print("\n" + "=" * 60)
    print("Gold Variations Renderer")
    print("=" * 60)
    ...
    print(f"\nRendering {len(GOLD_VARIATIONS)} variations...")
    print("-" * 40)

    rendered_files = []
    for i, variation in enumerate(GOLD_VARIATIONS, 1):
        print(f"\n[{i}/{len(GOLD_VARIATIONS)}] {variation['name']}")
